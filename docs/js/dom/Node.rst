...
.. js:class:: Node()
    ...
    .. js:attribute:: baseURI
        ...
        Оп­ре­де­ля­ет ба­зо­вый URL-ад­рес для дан­но­го объ­ек­та Node, ко­то­рый бу­дет ис­поль­зо­вать­ся при раз­ре­ше­нии от­но­си­тель­ных URL-ад­ре­сов. Для всех уз­лов в HTML-до­ку­мен­тах этот URL-ад­рес оп­ре­де­ля­ет­ся эле­мен­том <base> или свой­ст­вом Docu­ment.URL, из зна­че­ния ко­то­ро­го ис­клю­ча­ет­ся иден­ти­фи­ка­тор фраг­мен­та.


    .. js:attribute:: childNodes

        список :js:class:`NodeList` детей данного узла
    ...
    .. js:attribute:: nextSibling
        ...
        следующий элемент :js:class:`Node`, дочерний элемент того же родительского элемента
    ...
    .. js:attribute:: previousSibling
        ...
        предыдущий дочерний элемент , обладающий тем же родительским элементом что и данный узел
    ...
    .. js:function:: appendChild(node)
        ...
        добавляет узел - наследник. Привязывает новый узел к дереву , ставя его последним в списке дочерних узлов данного узла


    .. js:function:: cloneNode(deep)

        Соз­да­ет и воз­вра­ща­ет ко­пию уз­ла, для ко­то­ро­го он вы­зван.
    ...
    .. js:function:: insertBefore(newNode, refNode)
        ...
        вставляет новый узел, newNode, перед существующим узлом refNode
    ...
    .. js:function:: removeChild(node)
        ...
        удаляет указанный узел - наследник


    .. js:function:: replaceChild(newNode, oldNode)

        заменяет старый узел oldNode новым узлом newNode