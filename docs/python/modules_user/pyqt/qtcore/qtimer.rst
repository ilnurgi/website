...
.. py:class:: QTimer((parent=None))
    ...
    .. py:method:: setInterval(interval)
        ...
        Задает промежуток времени в миллисекундах, по истечении которого генерируется сигнал :py:method:`timeout()`. Минимальное значение интервала зависит от операционной системы. Если в параметре `interval` указать значение 0, то таймер бу­дет срабатывать много раз при отсутствии других необработанных сигналов.
    ...
    .. py:method:: start([interval])
        ...
        Запускает таймер. В необязательном параметре можно указать промежуток времени в миллисекундах. Если параметр не указан, то используется значе­ние, возвращаемое методом :py:method:`interval()`
