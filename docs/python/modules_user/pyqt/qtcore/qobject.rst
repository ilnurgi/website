...
.. py:class:: QObject()
  ...
  базовый класс для всех объектов модуля
    ...
    .. py:method:: startTimer(interval)
      ...
      * *interval* - задает промежуток времени в миллисекундах, по истечении которого выполняется метод :py:method:`timerEvent()`. Если равен нуля, то таймер выполняется бесконечное количество раз
